...
namespace phoeg
{
     ...
     */
    string convertToGraph6(const Graph & g)
    {
        std::string g6 = encodeOrder(order(g));
        if (numEdges(g) == 0)
        {
            long l = lengthGraph6(order(g));
        ...
        else
        {
            ...
            {
                for (std::pair<vertex_iter,vertex_iter> v = vertices(g); v.first != u.first; ++v.first)
                {
                    if (l == 6)
                    {
                        g6 += (char) r+63;
    ...
    long decodeOrderGraph6(std::string & g6)
    {
        ...
        {
            n = (g6[2]-63)<<30 + (g6[3]-63)<<24 + (g6[4]-63)<<18 + (g6[5]-63)<<12 + (g6[6]-63)<<6 + (g6[7]-63);
     ...
     */
    Graph convertFromGraph6(const string & s)
    {
        string g6(s);
        long n = decodeOrderGraph6(g6);
        Graph g(n);
        if (g6.size() > 0)
        {
            char v = g6[0]-63;
